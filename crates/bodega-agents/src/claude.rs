...
//!
//! Bodega chooses the session id (`--session-id`), passes untrusted values
...
use bodega_core::{AgentEvent, LogLevel, Usage};
...
/// types are skipped so newer CLIs do not break older Bodega versions.
                            ...
                            let _ = write_json(
                                ...
                                &json!({
                                    ...
                                    "response": {
                                        ...
                                        "error": format!("bodega does not support `{subtype}` requests"),
                                    },
                                }),
                            )
                            .await;
...
impl AgentControl for ClaudeControl {
    ...
    async fn interrupt(&self) -> Result<()> {
        ...
        write_json(
            ...
            &json!({
                "type": "control_request",
                "request_id": format!("bodega-interrupt-{n}"),
