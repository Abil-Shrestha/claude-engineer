//! Forgeline's HTTP API: the contract every UI, bot and script builds on.
//!
...
use forgeline_config::Plan;
use forgeline_core::{ApprovalId, ApprovalState, Decision, Event, RunId, RunState, WorkSource};
...
impl From<forgeline_store::StoreError> for ApiError {
    fn from(e: forgeline_store::StoreError) -> Self {
        match e {
            forgeline_store::StoreError::Rejected(e) => {
...
impl From<forgeline_engine::EngineError> for ApiError {
    fn from(e: forgeline_engine::EngineError) -> Self {
        use forgeline_engine::EngineError as E;
        let status = match &e {
            E::Plan(_) | E::UnknownRuntime(..) | E::Git(_) => StatusCode::BAD_REQUEST,
            E::UnknownRun(_) => StatusCode::NOT_FOUND,
            E::Store(forgeline_store::StoreError::Rejected(_)) => StatusCode::CONFLICT,
...
fn parse_run_id(raw: &str) -> ApiResult<RunId> {
    raw.parse()
        .map_err(|e: forgeline_core::ParseIdError| ApiError(StatusCode::BAD_REQUEST, e.to_string()))
}
...
async fn resolve_approval(
    ...
    let approval_id: ApprovalId = id.parse().map_err(|e: forgeline_core::ParseIdError| {
