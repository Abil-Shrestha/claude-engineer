...
use forgeline_config::{Plan, PlanTask};
use forgeline_core::{
...
use forgeline_server::{CreateRun, Created, PendingApproval, Resolve};
use forgeline_store::RunSummary;
use ts_rs::{Config, TS};
...
fn typescript_types_are_up_to_date() {
    ...
    assert!(
        ...
        "{} is out of date with the Rust types.\n\
         Regenerate it with: UPDATE_TYPES=1 cargo test -p forgeline-server --test typescript",
        path.display()
    );
}
