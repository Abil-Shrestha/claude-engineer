//! `bodega`: run swarms of coding agents from the terminal.
...
use bodega_agents::{AgentRuntime, ClaudeCode, MockAgent};
use bodega_config::{AgentConfig, CONFIG_FILE, Config, Plan, TEMPLATE};
use bodega_core::{ApprovalId, Decision, RunId, RunState, WorkSource};
use bodega_engine::{Engine, NewRun};
use bodega_store::EventStore;
use bodega_workspace::GitRepo;
...
#[derive(Subcommand)]
enum Command {
    /// Set up Bodega in this repository (bodega.toml, .gitignore).
    Init,
    ...
    /// Check that everything Bodega needs is installed and configured.
    Doctor,
    ...
    Serve {
        ...
        /// Require this bearer token on every request (or BODEGA_TOKEN).
        #[arg(long, env = "BODEGA_TOKEN", hide_env_values = true)]
        token: Option<String>,
...
async fn main() {
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_env("BODEGA_LOG")
...
async fn dispatch(cli: Cli) -> Result<()> {
    ...
    match cli.command {
        ...
        Command::Runs => {
            ...
            if runs.is_empty() {
                outln!("No runs yet. Start one with `bodega run \"…\"`.");
            }
            for run in runs {
                outln!(
                    "{}  {:<22}  {}/{} tasks  {}  {}",
                    ui::dim(&bodega_engine::short_id(run.run_id)),
...
impl Project {
    ...
    /// Accepts a full run id or the short id shown by `bodega runs`.
    async fn resolve_run(&self, input: &str) -> Result<RunId> {
        ...
        let matches: Vec<RunId> = self
            ...
            .into_iter()
            .map(|r| r.run_id)
            .filter(|id| bodega_engine::short_id(*id).starts_with(input))
            .collect();
        match matches.as_slice() {
            [id] => Ok(*id),
            [] => bail!("no run matches `{input}` (see `bodega runs`)"),
...
/// Reads `bodega.toml` from the base branch, so agents cannot change their
...
async fn follow_drive(engine: &Engine, run_id: RunId, verbose: bool) -> Result<()> {
    ...
    let result = tokio::select! {
        ...
        _ = tokio::signal::ctrl_c() => {
            printer.abort();
            outln!(
                "\n{} agents stopped. Continue with `bodega resume {}`.",
                ui::yellow("interrupted:"),
                bodega_engine::short_id(run_id)
            );
            return Ok(());
        }
    };
...
fn print_run(state: &RunState) {
    ...
    for task_id in order {
        ...
        for attempt_id in &task.attempts {
            ...
            let outcome = match &attempt.outcome {
                ...
                Some(bodega_core::AttemptOutcome::Succeeded { .. }) => ui::green("verified"),
                Some(bodega_core::AttemptOutcome::Failed { kind, .. }) => {
                    ui::red(&format!("failed ({kind:?})"))
                }
                Some(bodega_core::AttemptOutcome::Cancelled) => ui::dim("cancelled"),
            };
            outln!(
                "  {:<key_width$}  #{} {}  {}  {}  {}",
                "",
    ...
    if state.status == bodega_core::RunStatus::Succeeded {
...
async fn init(start: &Path) -> Result<()> {
    let repo = GitRepo::open(start)
        .await
        .context("run `git init` first: Bodega works inside a git repository")?;
    let root = repo.root();
    ...
    if !existing.lines().any(|l| l.trim() == ".bodega/") {
        let mut text = existing;
        if !text.is_empty() && !text.ends_with('\n') {
            text.push('\n');
        }
        text.push_str("\n# Bodega state (database, agent worktrees)\n.bodega/\n");
        std::fs::write(&gitignore, text)?;
        outln!("{} .bodega/ to .gitignore", ui::green("added"));
    }
    outln!(
        "\nNext:\n  1. Edit {CONFIG_FILE}: add your [[checks]] (build, tests) and pick an agent.\n  \
         2. Commit it: git add {CONFIG_FILE} .gitignore && git commit -m \"Add Bodega\"\n  \
         3. Try it without spending tokens: bodega run \"Say hello\" --agent mock\n  \
         4. Then for real: bodega run \"<what to build>\" [--plan plan.toml]"
    );
    Ok(())
}
...
async fn doctor(start: &Path) -> Result<()> {
    ...
    if config.checks.is_empty() {
        warn(
            "checks",
            "none configured: work is integrated without verification. Add [[checks]] to bodega.toml.",
        );
    } else {
    ...
    if ignored {
        ...
        warn(
            "state directory",
            &format!(
                "add `{}/` to .gitignore (bodega init does this)",
