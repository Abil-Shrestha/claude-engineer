...
//!
//! Forgeline shells out to `git` instead of linking libgit2 so that behaviour
...
/// Identity used for commits Forgeline makes on behalf of agents.
pub const COMMITTER_NAME: &str = "Forgeline";
pub const COMMITTER_EMAIL: &str = "forgeline@localhost";
...
/// A repository Forgeline operates on (the developer's checkout).
#[derive(Debug, Clone)]
pub struct GitRepo {
    root: PathBuf,
}
