...
use bodega_core::{ApplyError, Event, EventKind, RunId, RunState, RunStatus, Usage};
...
pub enum StoreError {
    ...
    UnknownRun(RunId),
    #[error("database was created by a newer Bodega (schema {found}, supported {supported})")]
    ...
    fn of(state: &RunState) -> Self {
        Self {
            ...
            tasks_done: state
                .tasks
                .values()
                .filter(|t| t.status == bodega_core::TaskStatus::Done)
                .count(),
...
mod tests {
    use super::*;
    use bodega_core::{RunSpec, TaskId, TaskSpec, TaskStatus, WorkSource};
    ...
    async fn two_handles_on_one_file_stay_consistent() {
        ...
        // example the engine and `bodega approve`).
    ...
    async fn state_survives_reopen() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("bodega.sqlite");
        let run = RunId::new();
        let t1 = TaskId::new();
        {
