//! Forgeline configuration.
//!
//! A repository opts in with a `forgeline.toml` at its root. The engine reads
...
use forgeline_core::{DepGraph, GraphError, TaskSpec};
use serde::{Deserialize, Serialize};
use serde_json::Value;

/// File name looked up at the repository root.
pub const CONFIG_FILE: &str = "forgeline.toml";

/// A commented starter config written by `forgeline init`.
pub const TEMPLATE: &str = r#"# Forgeline configuration. Read from the base branch, so agents cannot change
...
[agents.default]
runtime = "claude-code"      # or "mock" to try Forgeline without spending tokens
# model = "claude-opus-5"
...
# else is escalated to a human (`forgeline approvals`, `forgeline approve`).
[permissions]
...
pub struct Project {
    ...
    /// Where Forgeline keeps its database and worktrees, relative to the
    /// repository root.
...
mod tests {
    ...
    fn template_parses_and_defaults_apply() {
        ...
        assert_eq!(config.project.state_dir, ".forgeline");
