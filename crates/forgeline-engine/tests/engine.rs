...
impl Harness {
    async fn new(config: &str, agent: MockAgent) -> Self {
        ...
        let store = EventStore::open(dir.path().join("forgeline.sqlite")).unwrap();
...
fn task_id_by_key(state: &RunState, key: &str) -> forgeline_core::TaskId {
...
async fn unknown_tool_requests_wait_for_a_human() {
    ...
    // Wait for the escalation, then approve it like `forgeline approve` would.
        ...
        .append(
            ...
            vec![
                ...
                EventKind::AttemptStarted {
                    attempt_id: forgeline_core::AttemptId::new(),
                    task_id,
                    number: 1,
                    agent: forgeline_core::AgentRef {
                        runtime: "mock".into(),
                        model: None,
                    },
                    workspace_id: forgeline_core::WorkspaceId::new(),
                    branch: "forgeline/dead/T1-1".into(),
                },
            ],
            None,
        )
