...
use forgeline_agents::{
    AgentRuntime, AgentSession, PermissionDecision, PermissionMode, StartRequest,
};
use forgeline_config::{
...
async fn decide_permission(
    ...
    match ctx.policy.decide(tool, input) {
        ...
        PolicyDecision::Deny { rule } => PermissionDecision::Deny {
            message: format!("Forgeline's permission policy denies this (rule `{rule}`)."),
            interrupt: false,
        },
        PolicyDecision::Escalate => {
            ...
            if let Err(e) = requested {
                ...
                return PermissionDecision::Deny {
                    message: "Forgeline could not ask a human for approval.".into(),
                    interrupt: false,
                };
            }
