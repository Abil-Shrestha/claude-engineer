...
//!    tasks it depends on.
//! 2. When an agent finishes, Forgeline commits its work and runs the
...
use forgeline_agents::AgentRuntime;
use forgeline_config::{Config, Plan};
use forgeline_core::{
...
use forgeline_store::{EventStore, StoreError};
use forgeline_workspace::{GitError, GitRepo, Worktree};
...
pub enum EngineError {
    ...
    Plan(#[from] forgeline_config::ConfigError),
...
impl Engine {
    /// `state_dir` holds worktrees (usually `<repo>/.forgeline`).
    ...
    pub fn integration_branch(run_id: RunId) -> String {
        format!("forgeline/{}/integration", short_id(run_id))
    }
                ...
                .append(
                    ...
                    vec![EventKind::AttemptFinished {
                        ...
                        outcome: AttemptOutcome::Failed {
                            kind: FailureKind::Infrastructure,
                            message:
                                "interrupted: Forgeline stopped while this attempt was running"
                                    .into(),
                        },
                        usage: attempt.usage,
                    }],
    ...
    async fn integrate(
        ...
        let failure = match integration
            .merge(&report.branch, &format!("forgeline: integrate {key}"))
            .await
        {
            Ok(merge_commit) => {
