...
```sh
cd your-project
bodega init          # writes bodega.toml, adds .bodega/ to .gitignore
```

Edit `bodega.toml`. The most important part is **checks**: the commands
...
Then commit it. Bodega reads `bodega.toml` **from the base branch**, so
an agent can never loosen its own checks or permissions by editing the file.

```sh
git add bodega.toml .gitignore && git commit -m "Add Bodega"
bodega doctor        # checks git, config, agents
```

## Your first run (no tokens spent)

```sh
bodega run "Say hello" --agent mock
```

You will see the task start, the checks run, and the work merge into the run's
integration branch. `bodega show <id>` prints the result; `git log
main..bodega/<id>/integration` shows the commits.
...
```sh
bodega run "Fix the flaky retry test in src/net.rs"
```
...
```sh
bodega run "Add dark mode" --plan plan.toml --max-cost 10 --verbose
```

What happens:

1. `tokens` and `toggle` start at once, each in its own worktree under
   `.bodega/worktrees/`, on its own branch.
2. When an agent finishes, Bodega commits its work and runs your checks.
...
3. Verified work is merged into `bodega/<id>/integration` one task at a
...
```sh
bodega approvals                          # what is waiting
bodega approve apr_0192…                  # allow it
bodega approve apr_0192… --deny --comment "use the test fixture instead"
```
...
|---|---|
| `bodega runs` | List runs with status, progress and spend |
| `bodega show <run>` | Tasks, attempts, checks and spend for one run |
| `bodega events <run> [--follow] [--json]` | The run's event log |
| `bodega resume <run>` | Continue after Ctrl-C, a crash or a reboot |
| `bodega serve` | HTTP API + live stream for the web UI ([API.md](API.md)) |
| `bodega doctor` | Check the setup |

Run ids can be shortened to the 8 characters `bodega runs` shows.

## Where things live

```
your-project/
├── bodega.toml            # committed: agents, checks, limits, permissions
└── .bodega/               # ignored: local state
    ├── bodega.sqlite      # the event log (everything that happened)
