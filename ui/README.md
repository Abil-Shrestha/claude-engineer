...
[Forgeline API](../docs/API.md): no engine code, no database access, just
...
- [`src/api/types.ts`](src/api/types.ts): TypeScript types for every request,
  ...
  `UPDATE_TYPES=1 cargo test -p forgeline-server --test typescript`
...
cd some-repo && forgeline init && git add -A && git commit -m forgeline
forgeline run "Try it" --agent mock
forgeline serve --allow-origin http://localhost:5173

# terminal 2: your dev server on :5173
```

`forgeline serve --ui <dist-dir>` serves a built UI from the same origin, so
